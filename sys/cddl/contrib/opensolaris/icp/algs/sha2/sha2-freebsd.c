...
{
	switch (mech) {
	case SHA256:	SHA256_Init(&ctx->sha.sha256); break;
	case SHA384:	SHA384_Init(&ctx->sha.sha384); break;
	case SHA512:	SHA512_Init(&ctx->sha.sha512); break;
...
SHA2Update(SHA2_CTX *ctx, const void *buf, size_t len)
{
	void *sha = &ctx->sha;
	switch (ctx->algotype) {
	case SHA256:	SHA256_Update(sha, buf, len); break;
	case SHA384:	SHA384_Update(sha, buf, len); break;
	case SHA512:	SHA512_Update(sha, buf, len); break;
...
SHA2Final(void *buf, SHA2_CTX *ctx)
{
	void *sha = &ctx->sha;
	switch (ctx->algotype) {
	case SHA256:	SHA256_Final(buf, sha); break;
	case SHA384:	SHA384_Final(buf, sha); break;
	case SHA512:	SHA512_Final(buf, sha); break;
	default:	panic("Unknown SHA2 type %u", ctx->algotype);
		break;
	}
	return;
