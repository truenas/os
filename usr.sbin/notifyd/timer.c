 ...
 */
static time_t
timer_next(timer_event_t *t, time_t now)
{
	uint32_t y, m;
...
timer_oneshot(time_t when, dispatch_queue_t queue)
{
	...
	t = calloc(1, sizeof(timer_event_t));
...
void
timer_set_deactivation_handler(timer_event_t *t, void(^handler)())
{
	if (t == NULL) return;
